...
sourceCpp("1_CDAMT_functions_v1.cpp")
...
cat("\nSearching for scale parameter.\n")
for(i in 1:length(mygrid)){
	pct			<- proc.time()
	scale_ls[[i]]<- mtretropC(param_init, mygrid[i], niter, 0, 1, 500, 
					 		hh_index, TT_vec, init_state, DataState, choice_seq, W, DP_init, control_list, 
							logit_pred, Q_obs, omega_dif, phat, nest_pred = nest_pred)
	use.time	<- proc.time() - pct
	cat("Sampling at scale paramter=", mygrid[i], "with", use.time[3]/60, "min.\n")
}
...
last.draw	<- scale_ls[[sel]]$draw[niter,]
niter		<- 11000
nburn		<- 1000
nthin		<- 40
print_freq	<- 50
...
pct			<- proc.time()
sol			<- mtretropC(last.draw, mcmc.scale, niter, nburn, nthin, print_freq, 
				 		hh_index, TT_vec, init_state, DataState, choice_seq, W, DP_init, control_list, 
						logit_pred, Q_obs, omega_dif, phat, nest_pred = nest_pred)
se.time	<- proc.time() - pct
