...
struct policy_fn{
	...
	struct spl *spl2d_h1; struct spl *spl2d_h2; struct spl *spl2d_h3; struct spl *spl2d_h4; 
	
	NumericVector policyEval(NumericVector s1, double y){
		int N1 = x1.length(), N2 = x2.length(), N3 = x3.length(), N4 = x4.length();
		NumericVector out(n_act + 5), y4(N4), c4(N4);
		NumericMatrix h4(N4, 4);
		double Q, hstar; 
		...
		if(y == -999){
			...
			for(int j=0; j<N4; j++){
				...
				h4(j,0)= spl2d_h1[index].splEval(s1(0));
				h4(j,1)= spl2d_h2[index].splEval(s1(0));
				h4(j,2)= spl2d_h3[index].splEval(s1(0));
				h4(j,4)= spl2d_h4[index].splEval(s1(0));								
			}
			...
			c_spl_new.splfree();
			for(int k=0; k<4; k++){
				struct spl h_spl_new = spl_init(x4, h4(_,k)); 
				out(3+k) = h_spl_new.splEval(s1(3));
				h_spl_new.splfree(); 
			}		
...
// [[Rcpp::export]]
NumericMatrix Comp_Lagrg(List DP_list, List control_list){
	// Unload parameters; 
	NumericVector param = DP_list["param"]; 
	NumericMatrix state = DP_list["state"]; 
	double beta 		= DP_list["beta"]; 
	Function omega_fn 	= DP_list["omega"], Q_fn = DP_list["Q_fn"], omega_j = DP_list["omega_j"], Q_j = DP_list["Q_j"]; 
	List Inc_kernel 	= DP_list["Inc_kernel"], lnZ_kernel = DP_list["lnZ_kernel"]; 
	NumericVector Inc_nodes = Inc_kernel["nodes"], 
			lnZ_nodes	= lnZ_kernel["nodes"], lnZ_weight = lnZ_kernel["weight"]; 
	NumericMatrix Inc_weight = Inc_kernel["weight"]; 
	...
	double lambda 		= param_out["lambda"], tau = param_out["tau"], mu = param_out["mu"], sigma = param_out["sigma"];
	NumericVector V_old	= DP_list["value_fn"]; 
	...
	NumericMatrix h(ns, 4); 
	NumericVector domega 	= omega_j(policy(_,0)); 
	NumericVector dQ 		= Q_j(policy(_,0)); 
	NumericVector Qvec		= Q_fn(policy(_,0));
	int n_Inc				= Inc_nodes.length(), n_Z = lnZ_nodes.length();  
	NumericVector y4(N4), dev_Inc(n_Inc), v(n_Z), lnZ_next = sqrt(2)*sigma*lnZ_nodes + mu; 
	double dev_Inext; 
	for(int i=0; i<ns; i++){
		// Compute dEV/dInext; 
		double Inext = state(i,0) + Qvec(i) - policy(i,1); 
		for(int j=0; j<n_Inc; j++){
			int x2_index = findInterval1(j+1, x2);
			int x3_index = findInterval1(state(i,2), x3);
	
			// Take derivative w.r.t. I_next; 	
			for(int k=0; k<N4; k++){
				int index = k*N2*N3 + (x3_index - 1)*N2 + x2_index - 1; 
				y4(k) = myspl[index].spldevEval(Inext);
			}
	
			// Create the spline interpolation along x4; 			
			struct spl newspl = spl_init(x4, y4);
			for(int k=0; k<n_Z; k++){
				v[k] = newspl.splEval(lnZ_next[k]);
			}
			newspl.splfree(); 
			dev_Inc(j) = sum(lnZ_weight*v)/sqrt(PI);		
		}
		NumericVector Inc_weight1 = Inc_weight((n_Inc*state(i,2) + state(i,1) - 1),_);
		dev_Inext 	= sum(dev_Inc * Inc_weight1);
		
		// Compute Lagrangian multiplier; 
		h(i,0)	= - domega(i) + 1 + tau*dQ(i) - beta*dQ(i)*dev_Inext; 
		h(i,1)	= -h(i,0);
		h(i,2)	= -lambda/(policy(i,1)+.01) - tau +beta*dev_Inext; 
		h(i,3)	= (1-1/dQ(i))*(lambda/(policy(i,1)+.01) + tau + domega(i) - 1 - tau*dQ(i)) - 
				   beta*dev_Inext*(2 - dQ(i) + 1/dQ(i)); 
	}
...
List policy_iterC(List DP_list, List control_list, int print_level=0){
	...
	NumericMatrix h			= Comp_Lagrg(DP_list, control_list); 
	DP_listout["h"]			= h;  
...
// 	struct policy_fn myplc = (struct policy_fn *)plcspl;
	...
	NumericMatrix h(DataState.nrow(),4); 
...
// 	}
	...
	return Rcpp::List::create(_["DataState"] = state_out, _["choice_seq"] = choice_out, _["h"] = h);
}
...
List sim_hhseqC(NumericVector hh_index, IntegerVector TT_vec, NumericMatrix init_state, NumericMatrix DataState, NumericMatrix choice_seq, List DP_list, List control_list, bool scalez=true){
	...
	// Initiate the c-spline; 
	int nspl = N2*N3*N4;
	struct spl y_spl[nspl], c_spl[nspl], h_spl1[nspl], h_spl2[nspl], h_spl3[nspl], h_spl4[nspl];
	NumericVector tmph(N1); 
	for(int i=0; i<nspl; i++){
		IntegerVector index	= i*N1 + seq_len(N1) - 1;
		...
		tmph 	= h_mat[index,0]; 
		h_spl1[i] = spl_init(x1, tmph); 
		tmph 	= h_mat[index,1]; 
		h_spl2[i] = spl_init(x1, tmph); 
		tmph 	= h_mat[index,2]; 
		h_spl3[i] = spl_init(x1, tmph); 
		tmph 	= h_mat[index,3]; 
		h_spl4[i] = spl_init(x1, tmph); 
		
	}
	
	// Initiate policy function structure; 
	struct policy_fn myplc = {n_act, Inc_nodes, Inc_weight, Q_fn, x1, x2, x3, x4, y_spl, c_spl, 
							 h_spl1, h_spl2, h_spl3, h_spl4};
	...
	return Rcpp::List::create(_["DataState"] = state_out, _["choice_seq"] = choice_out, _["h"]=h_out);
}
