...
List param_assign(NumericVector param){
	double lambda = param[0], tau1 = param[1], tau2 = param[2], tau3 = param[3], tau4 = param[4],
			lambda_o = param[5]; 
	return Rcpp::List::create(_["lambda"] = lambda, _["tau1"] = tau1, _["tau2"] = tau2,
							  _["tau3"] = tau3, _["tau4"] = tau4, _["lambda_o"] = lambda_o);
}
...
struct DP_fun{
	double omega_fn(double y, int d){
		double out = -0.015 * d * pow(y, 2) + 0.2 * d * y; 
		return(out); 
	}
	
	double Q_fn(double y){
		double out = .1 * y; 
		return(out); 
	}
};
...
// s1d is brent one-dimensional minimizer when y = 0, the function F1d is negative value function; 
	...
	// Return y, c and V(d, y, c); 
		...
		double upper_Q = DPf.Q_fn(upper_y);
...
// 			tau3				= param_out["tau3"], tau4 = param_out["tau4"], lambda_o = param_out["lambda_o"];
	double lambda				= param_vec[0], tau1 = param_vec[1], tau2 = param_vec[2], 
			tau3 				= param_vec[3], tau4 = param_vec[4], lambda_o = param_vec[5]; 
...
// 	double Q = DPf.Q_fn(y);
	...
	double d_n	= 0; 
	if(y > 0){ d_n = *d_ptr; }
	double omega = -0.015 * d_n * pow(y, 2) + 0.2 * d_n * y;
	out = omega + lambda*log(c+0.01) + lambda_o*(Inc-y-Z) - tau1*s_next[0] - tau2*pow(s_next[0],2) 
		+ tau3*s_next[0]*d_n - tau4*d_n + beta*ev; 
	return(out);
}	
...
double my_valuefn_t(const gsl_vector *a_t, void *p){
	...
	g(0) = y - (Inc - Z)*1*(Inc>Z); 
	g(1) = c - (I + Q); 
	double my_penalty = 1000.0, v, out, tol_g = 1e-6; 
	sign_g.fill(0); 
...
List policy_improveC(NumericVector V_old, List DP_list, List control_list, NumericMatrix policy_inits, bool inits_gen=false, double NM_sizetol=0, int NM_iter = 0){
	...
	int d = 1; 
	int *d_ptr = &d; 
	struct my_f_params params = {d_ptr, param, state(0,_),beta, DPf, lnzero, 
	...
	for(int i=0; i<ns; i++){
		...
		if(inits_gen){			
			...
			double Q = DPf.Q_fn(policy_inits(i,1)); 
...
List sim_hhseqC(NumericVector hh_index, IntegerVector TT_vec, NumericMatrix init_state, NumericMatrix DataState, NumericMatrix choice_seq, List DP_list, List control_list, int simidx=12, bool scalez=true){
	...
	int d = 1; 
	struct DP_fun DPf; 
	struct my_f_params params = {&d, param, state(0,_),beta, DPf, lnzero, 
								Inc_nodes, Inc_weight, lnZ_nodes, lnZ_weight, 
								 x1, x2, x3, x4, V_spl};	
	...
	struct policy_fn myplc = {n_act, Inc_nodes, Inc_weight, DPf, x1, x2, x3, x4, D, y_spl, c_spl, orig_func};
...
NumericVector MM1C(NumericVector param, NumericMatrix choice_obs, NumericMatrix choice_seq_sim2, NumericMatrix DataState, IntegerVector TT_vec, List DP_list, Function logit_pred, NumericVector Q, NumericMatrix omega_mat){
	...
	List param_out 		= param_assign(param); 
	double lambda 		= param_out["lambda"], tau1 = param_out["tau1"], tau2= param_out["tau2"], 
			tau3		= param_out["tau3"], tau4 = param_out["tau4"];
	int D				= DP_list["D"]; 
	...
	for(int i=1; i<D; i++){
		md_mat(_,i-1) 			= omega_mat(_,i-1) - omega_mat(_,D-1) + tau3 * Inext*(i-D) - tau4*(i-D) - phat(_,i-1); 
		md 						= md + md_mat(_,i-1); 
	}
	...
	for(int i=0; i<n_inst; i++){
		...
		M(_,i+n_inst*2)	= pow(my,2)*inst(_,i);  
	}
...
NumericVector valueinitC(List DP_list, List control_list){
	...
	struct DP_fun DPf; 		
	NumericVector Inc(ns), Z(ns), omega(ns), d(ns), y(ns), c(ns), Q(ns); 
	Z			= exp(state(_,3)); 
	d.fill(1);
	for(int i=0; i<ns; i++){
