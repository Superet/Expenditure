...
tmp			<- tmp[,list(price = mean(price_paid_norm_index, na.rm=T)), by=list(year, month, channel_type)]
...
for(i in 1:length(unique(sel[,2]))){
	...
	tmp1		<- merge(subset(tmp, channel_type==fmt_name[i]), price[sel2,1:3], by=c("year","month"))
	tmp2		<- as.vector(tmp1$price)
	names(tmp2)	<- with(tmp1, paste(scantrack_market_descr, year, month,sep="-"))
	price[sel2, (sel1+3)] <- tmp2[with(price[sel2,], paste(scantrack_market_descr, year, month,sep="-"))]
}
...
for(j in 1:length(bskt_type)){
	bskt_idx<- j
	R		<- length(fmt_name)
	my_data1<- subset(my_data, basket_type == bskt_idx & !is.na(dol_purchases))
...
for(k in 1:length(bskt_type)){
	MDCEV_wrapper <- function(param){
		MDCEV_LogLike_fnC(param, nx, all_e1_index[[k]], all_eR[[k]], all_price[[k]], all_X_list[[k]])
	}
	
	# Estimation with multiple initial values. 
	theta_init	<- list(c(.1, .5, 1, -.5, 1, -1, 1, 1, -1, 1), 
						c(-.2, 0, .5, -.1, 1, -1, 1, 1, -2, 1),
						c(-3, -1, -5, -2, 1, -.5, 1, 2, -2, 1),
						c(.5, -2, -2, -2, 5, -5, 5, 5, -5, 5)  ) 
...
# #--------------------------------------------------- # 
# # Compute the inclusve value within each market-year # 
