...
gamma0	<- c(1, 1, 1)
gamma 	<- gamma0
if(substr(model_name,6,7)=="a2"){
	Ra 	<- Ra + 1
	exp_outside <- TRUE
	gamma	<- c(gamma, 1)
}
if(substr(model_name,8,9)=="b2"){
	Ra 	<- Ra + 1
	quant_outside <- TRUE
	gamma <- c(gamma, 1)
}
...
eps_draw<- matrix(rnorm(N*R, sd = sigma), N, R)
# eps_draw<- cbind(0, matrix(rgumbel(N*(R-1)), N, R-1))
...
# Estimation 
if(substr(model_name,8,9)=="b2"){
	MDCP_wrapper <- function(param){
		MDCP_LogLike_fnC(param, nx, c_q = qz_cons, y = y, Q= Q, e= eR, p=price, X_list = X_list)
	}
	param 	<- c(beta, beta0, gamma[1:R], log(psi_R1), log(psi_R2), log(sigma)) 
}else{
	MDCP_wrapper <- function(param){
		MDCP_LogLike_fnC(param, nx,y = y, e= eR, p=price, X_list = X_list)
	}
	param 	<- c(beta, beta0[-1])
}
...
system.time(tmp <- MDCP_wrapper(theta_init[[1]]) )
...
for(i in 1:length(theta_init)){
	...
	tmp_sol[[i]]	<- maxLik(MDCP_wrapper, start=theta_init[[i]], method="BFGS")
}
...
for(i in 1:length(theta_init)){
	names(theta_init[[i]]) <- c(paste("beta_",1:nx, sep=""), paste("beta0_",1:R,sep=""), paste("gamma",1:R, sep=""),
								"ln_psi1", "ln_psi2", "ln_sigma")	
	tmp_sol[[i]]	<- maxLik(MDCP_wrapper, start=theta_init[[i]], method="BFGS", fixed = myfix)
}
lapply(tmp_sol, summary)
