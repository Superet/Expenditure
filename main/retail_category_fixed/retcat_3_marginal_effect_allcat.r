...
draw.par		<- FALSE
sim.omega		<- FALSE
fname			<- "retcat_marg_effct_allcat"
...
pct			<- proc.time()
sim.ls07	<- foreach(v = c(selcol, "price","distance"), .errorhandling = "remove", .packages=c("mgcv", "nloptr")) %dopar% {
	out1	<- data.frame()
	price.new	<- price
	attr.new	<- attr_sel
	X1.new		<- X1.07
	if(v == "price"){
		price.new	<- price.new*(1 + iv.change)
	}else if(v %in% c("ln_upc_per_mod", "ln_num_module")){
		sel	<- grep(v, colnames(attr.new))
	...
	if(v == "distance"){
		sel	<- grep("distance", colnames(X1.new[[sel.retail]]))
		X1.new[[sel.retail]][,sel]	<- X1.new[[sel.retail]][,sel] * (1 + iv.change)
	}
	...
	for(i in 1:nseg){
		...
		tmp		<- SimWrapper_fn(lambda[,i], Inc = sim.unq[sel,"income"], X = lapply(X1.new, function(x) x[sel,]), 
...
for(i in 1:length(sim.ls07)){
	tmp1[[i]]	<- (sim.ls07[[i]][,sel.retail] - base.07[,sel.retail])/base.07[,sel.retail]/sim.ls07[[i]][,"change"]
	tmp2[[i]]	<- (sim.ls08[[i]][,sel.retail] - base.08[,sel.retail])/base.08[,sel.retail]/sim.ls08[[i]][,"change"]
	sel	<- !(is.na(tmp1[[i]]) | is.na(tmp2[[i]]) | abs(tmp1[[i]]) == Inf | abs(tmp2[[i]]) == Inf)
	tmp0	<- t.test(tmp2[[i]][sel], tmp1[[i]][sel])
	tmp		<- rbind(tmp, c(tmp0$estimate, tmp0$p.value))
}
dimnames(tmp)	<- list(names(sim.ls07), c("2008", "2007", "pvalue"))
