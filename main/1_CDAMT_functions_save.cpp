...
// 	double omega = DPf.omega_fn(y, d_n); 
	...
	if(!std::isfinite(out) || abs(out) > 999999){
		out = 999999;
	}
	return(out);
}	
...
//-------------------------------------------------//;
List policy_improveC(NumericVector V_old, List DP_list, List control_list, NumericMatrix policy_inits, struct DP_fun DPf, bool inits_gen=false, double NM_sizetol=0, int NM_iter = 0){
//NOTE: check the Q_fn when generating initial values; 
	...
	double NM_startsize		= control_list["NM_startsize"]; 
	gsl_set_error_handler_off();
...
List policy_iterC(List DP_list, List control_list, int print_level=0, int init_V=1){
	...
	double dif 	= tol + 1, NM_sizetol = 1e-3; 
	...
	List tmp_sol = policy_improveC(V0, DP_list, control_list, policy_new, DPf, true, NM_sizetol, NM_iter);
	...
	do{
		...
		tmp_sol					= policy_improveC(V_old, DP_list, control_list, policy_old, DPf, inits_gen, NM_sizetol, NM_iter);
