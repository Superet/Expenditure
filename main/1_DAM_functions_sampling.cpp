...
// [[Rcpp::export]]
List GMM_objC(NumericVector param_conv, NumericVector hh_index, IntegerVector TT_vec, NumericMatrix init_state, NumericMatrix DataState, NumericMatrix choice_obs, NumericMatrix W, List DP_init, List control_list, Function logit_pred, NumericVector Q, NumericMatrix omega_dif, NumericMatrix phat, bool nest_pred = false){	
	NumericVector param(param_conv.length()); 
	param(0) 	= exp(param_conv(0)); 
	param(1) 	= exp(param_conv(1)); 
	param(2) 	= param_conv(2); 
	param(3) 	= exp(param_conv(3)); 
	param(4) 	= -exp(param_conv(4)); 
	param(5) 	= param_conv(5); 
...
// [[Rcpp::export]]
List mtretropC(NumericVector inits, double scale, int niter, int nburn, int nthin, int print_freq, 
				NumericVector hh_index, IntegerVector TT_vec, NumericMatrix init_state, NumericMatrix DataState, 
				NumericMatrix choice_obs, NumericMatrix W, List DP_init, List control_list, 
				Function logit_pred, NumericVector Q, NumericMatrix omega_dif, NumericMatrix phat, bool nest_pred = false)
{
	// Initiate sampling 
	IntegerVector conv_idx	= IntegerVector::create(0, 1, 3, 4); 
	int neg_idx				= 2; 								// tau2 < 0, index in conv_idx; 
	int np 					= inits.length(), cnt = 0; 
	int nsample 			= floor( (niter - nburn)/nthin ), nfirst = floor(nburn/nthin); 
	double p_old, p_new, ll_old, ll_new, alpha, draw_alpha; 
	NumericVector theta_old(np), theta_new(np), prior_p(np);
	NumericVector ll_vec(nsample), p_vec(nsample), accept(niter) ; 
	NumericMatrix chain(nsample, np); 
	NumericVector J(conv_idx.length()); 
	...
	gsl_rng * rd_norm;
	gsl_rng * rd_unif; 
	rd_norm = gsl_rng_alloc (Tran);
	rd_unif	= gsl_rng_alloc (Tran);
	
	// Set prior; 
	double sigma = 10; 
	
	// Compute log(p(theta|y)) at initial values; 
	for(int k = 0; k < np; k++){
		theta_old(k)	= inits(k); 
		prior_p(k) 		= gsl_ran_gaussian_pdf (theta_old(k), sigma); 
	}
	List out_list 		= GMM_objC(theta_old, hh_index, TT_vec, init_state, DataState, 
									choice_obs, W, DP_init, control_list, logit_pred, Q, 
									omega_dif, phat, nest_pred); 
	ll_old				= out_list["obj"]; 
	if(ll_old == NA_REAL) {ll_old = 1E20; }
	J 					= theta_old[conv_idx]; 
	J[neg_idx]			= -J[neg_idx]; 
	p_old 				= sum(prior_p) + sum(J) - ll_old; 
	...
	// Sampling; 
	for(int i=0; i < nsample + nfirst; i++){
		for(int j=0; j < nthin; j++){
			// Draw new parameters and compute prior density; 
			for(int k = 0; k < np; k++){
				theta_new(k) 	= theta_old(k) + gsl_ran_gaussian (rd_norm, scale); 
				prior_p(k) 		= gsl_ran_gaussian_pdf (theta_new(k), sigma); 
			}
		
			// Evaluate p(theta|y) 
			out_list 			= GMM_objC(theta_new, hh_index, TT_vec, init_state, DataState, 
									choice_obs, W, DP_init, control_list, logit_pred, Q, 
									omega_dif, phat, nest_pred); 
			ll_new				= out_list["obj"]; 
			if(ll_new == NA_REAL) {ll_new = 1E20; }
			J 					= theta_new[conv_idx]; 
			J[neg_idx]			= -J[neg_idx]; 
			p_new 				= sum(prior_p) + sum(J) - ll_new; 
			alpha				= std::min(1.0, exp(p_new - p_old)); 
		
			// Accept the new draw if alpha is large;
			draw_alpha 			= gsl_ran_flat(rd_unif, 0.0, 1.0);
			if( draw_alpha < alpha){
				accept(i*nthin+j) 		= 1; 
				p_old 			= p_new; 
				theta_old		= theta_new; 
		...
		if(i >= nfirst){
			ll_vec(i)			= -ll_old; 				
			p_vec(i-nfirst)		= p_old; 
			chain(i-nfirst,_) 	= theta_old; 
		}  
		if((i+1) % print_freq == 0){
			endtime				= clock(); 
			use_time			= double(endtime - starttime)/ CLOCKS_PER_SEC;
			IntegerVector sel	= seq_len(print_freq); 
			sel					= cnt + sel -1; 
			NumericVector subacc= accept[sel];
			double acpt			= mean(subacc); 
			Rcout<<"Iteration sample "<<cnt+1<<"-"<<cnt+print_freq*nthin<<", acceptance = "<<acpt<<", using "<<use_time<<"sec.\n"; 
			cnt					= cnt + print_freq*nthin; 
		}
	}
		
	// Compute DIC; 
	double Dbar 		= mean(-2 * ll_vec); 
	NumericVector theta_bar(np); 
	for(int k = 0; k < np; k++){
		theta_bar(k)	= mean(chain(_,k)); 
	}
	out_list 			= GMM_objC(theta_bar, hh_index, TT_vec, init_state, DataState, 
									choice_obs, W, DP_init, control_list, logit_pred, Q, 
									omega_dif, phat, nest_pred); 
	double Dtilde		= out_list["obj"]; 
	Dtilde				= -1 * Dtilde; 
	double pD			= Dbar - Dtilde; 
	double DIC 			= pD + Dbar; 	
	NumericVector deviance = -2 * ll_vec;
	double accpet_r	= mean(accept); 		 			
	
	// Free memory; 
	gsl_rng_free (rd_norm);
	gsl_rng_free (rd_unif);
	
	return Rcpp::List::create(_["draw"] = chain, _["deviance"] = deviance, _["accept"] = accpet_r, 
							  _["Dbar"] = Dbar, _["pD"] = pD, _["DIC"] = DIC);
}
