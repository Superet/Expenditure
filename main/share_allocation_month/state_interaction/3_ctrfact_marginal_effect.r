...
for(i in 1:R){
	...
	X_list07[[i]]	<- cbind(tmp1, lag = shr.stat[,i], rep(1, nrow(sim.unq)) %*% matrix(X_list0[[i]], nrow = 1), matrix(0, nrow(sim.unq), length(selcol)))
	X_list08[[i]]	<- cbind(tmp2, lag = shr.stat[,i], rep(1, nrow(sim.unq)) %*% matrix(X_list0[[i]], nrow = 1), rep(1, nrow(sim.unq)) %*% matrix(X_list0[[i]], nrow = 1))
