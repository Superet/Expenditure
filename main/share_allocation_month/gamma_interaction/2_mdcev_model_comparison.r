...
if(cpi.adj){
	fname	<- paste("estrun_",run_id,"/MDCEV_intercept",seg_id,"_", Sys.Date(),".rdata",sep="")
}else{
	fname	<- paste("estrun_",run_id,"/MDCEV_intercept",seg_id,"_", Sys.Date(),".rdata",sep="")
}
fname
...
theta_init	<- list(c( -1, -.8, -1, -.4,-1.2, 1.5, 3.5, 2.5, 2.5, 3.5, 4.5,-.45), 
					c(-1, -.7, -1.2, -.3,-.8, 1.8, 3.5, 2.1, 2.1,3,4, -.5),
					c(.7, -.5, 1.4, -1.2,-.3, 1.6, 4, 2, 2.2, 3.2,5, -.5))
for(i in 1:length(theta_init)){ 
	names(theta_init[[i]])	<- c(paste("beta_", 1:nx, sep=""), paste("gamma0_", 1:R, sep=""), "ln_sigma")
}
...
omega_parallel	<- function(eps_draw){
	...
	for(k in 1:R){
		...
		tmp1[k]		<- 1
		tmpX1[[k]]	<- rep(1, numnodes) %*% t(tmp)
		tmpX2[[k]]	<- rep(1, numnodes) %*% t(tmp1)
	}
