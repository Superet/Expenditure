...
sim.ls07	<- foreach(v = c(selcol, "price"), .errorhandling = "remove", .packages=c("mgcv", "nloptr")) %dopar% {
	...
	out1	<- SimWrapper_fn(omega_deriv = omega_deriv, ln_inc = sim.unq[,"Inc07"], rec = rep(0, nrow(sim.unq)),
							...
							X1 = X1_new, X2 = X2_new, price = price_new, sim.y = sim.base07$y, 
...
sim.ls08	<- foreach(v = c(selcol, "price"), .errorhandling = "remove", .packages=c("mgcv", "nloptr")) %dopar% {
	...
	if(v == "price"){
		...
		X1_new[[sel.retail]][,v]	<- X1_new[[sel.retail]][,v] + log(1 + iv.change)
		X1_new[[sel.retail]][,paste("rec*",v,sep="")]	<- X1_new[[sel.retail]][,paste("rec*",v,sep="")] + log(1 + iv.change)
		X2_new[[sel.retail]][,v]	<- X2_new[[sel.retail]][,v] + log(1 + iv.change)
		X2_new[[sel.retail]][,paste("rec*",v,sep="")]	<- X2_new[[sel.retail]][,paste("rec*",v,sep="")] + log(1 + iv.change)
	}else{
		X1_new[[sel.retail]][,v]	<- X1_new[[sel.retail]][,v] * (1 + iv.change)
		X1_new[[sel.retail]][,paste("rec*",v,sep="")]	<- X1_new[[sel.retail]][,paste("rec*",v,sep="")] * (1 + iv.change)
		X2_new[[sel.retail]][,v]	<- X2_new[[sel.retail]][,v] * (1 + iv.change)
		X2_new[[sel.retail]][,paste("rec*",v,sep="")]	<- X2_new[[sel.retail]][,paste("rec*",v,sep="")] * (1 + iv.change)
	}
	
	out1	<- SimWrapper_fn(omega_deriv = omega_deriv1, ln_inc = sim.unq[,"Inc08"], rec = rep(0, nrow(sim.unq)),
							lambda = lambda, param_est = shr.par, base = beta0_base, 
							X1 = X1_new, X2 = X2_new, price = price_new, sim.y = sim.base08$y, 
							eps_draw = eps_draw, method = exp.method, ret.sim = TRUE, par.draw = par_draw, share.state = shr.stat[,-beta0_base])
...
tmp		<- setNames(tmp1[sel.retail,] - tmp0[sel.retail], selcol1)
cat("Impact on exp. share in 2007:\n"); print(tmp); cat("\n")
cat("Elasticity in 2007:\n"); print(tmp/tmp0[sel.retail]/iv.change); cat("\n")

tmp0	<- sim.base08$Average[1,fmt_name]/sim.base08$Average[1,"Exp"]
tmp1	<- sapply(sim.ls08, function(x) as.matrix(x$Average[1,gsub(" ", ".", fmt_name)]/x$Average[1,"Exp"]))
tmp		<- setNames(tmp1[sel.retail,] - tmp0[sel.retail], selcol1)
cat("Impact on exp. share in 2008:\n"); print(tmp); cat("\n")
cat("Elasticity in 2008:\n"); print(tmp/tmp0[sel.retail]/iv.change); cat("\n")
