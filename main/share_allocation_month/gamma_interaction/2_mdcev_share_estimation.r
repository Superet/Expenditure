...
theta_init	<- list(c( -1, -.8, -1, -.4,-1.2,	.2, .03, -.2, -.06, .1, 			.2, .5, .2, 2.3,	-.1, .1, -.2,-.6, 
					  1.5, 3.5, 2.5, 2.5, 3.5, 4.5,		-.2, -.5, -.5, .2, 0, 1,	0,.1,-.1,-2.5, 		-.1,.1,.1,1.5,	 	-.45), 
					c(-1, -.7, -1.2, -.3,-.8,	.2, .05,.4,.1,.5,					.1, .6, .1,2, 		-.05, .1, -.15, -.5,
					  1.8, 3.5, 2.1, 2.1,3,4, 			-.1, -.7, .1, .1, 0, 1.5,	0,.1,0,-2.7, 		0, .2,-.3,1.5,	-.5),
					c(.7, -.5, 1.4, -1.2,	-.3,-.12,-.38,-.15,.1,	1.8, .1, .4, .37, -.3, 	rep(-.1, length(selcol)),
					  1.6, 4, 2, 2.2, 3.2,5, 	0, .5, .1, .1, -.2, 3,	rep(-.1, length(selcol)*2),	-.5))
for(i in 1:length(theta_init)){ 
	names(theta_init[[i]])	<- c(paste("beta_", 1:nx, sep=""), paste("gamma0_", 1:((R+length(selcol))*2), sep=""), "ln_sigma")
}
...
omega_parallel	<- function(eps_draw, rec){
	tmpX1	<- tmpX2	<- vector("list", R)
	for(k in 1:R){
		tmp	<- rep(0, (R-1)*2)
		if(k < beta0_base){	tmp[k] <- 1; tmp[(R-1+k)]	<- rec}
		if(k > beta0_base){ tmp[(k-1)]	<- 1; tmp[(R-1+k-1)]	<- rec}
		tmp1		<- rep(0, R*2)
		tmp1[k]		<- 1
		tmp1[(R+k)]	<- rec
		tmpX1[[k]]	<- rep(1, numnodes) %*% t(c(tmp, tmpX_list[[k]], tmpX_list[[k]]*rec))
		tmpX2[[k]]	<- rep(1, numnodes) %*% t(c(tmp1, tmpX_list[[k]], tmpX_list[[k]]*rec))
	}
...
omega_draw	<- foreach(i = 1:numsim, .packages= c("nloptr"), .combine = 'rbind') %dopar% {
	omega_parallel(eps_draw[i,], rec = 0)
}
omega_draw1	<- foreach(i = 1:numsim, .packages= c("nloptr"), .combine = 'rbind') %dopar% {
	omega_parallel(eps_draw[i,], rec = 1)
}
...
if(interp.method == "spline"){
	...
	tmpdat	<- melt(omega_draw1)
	names(tmpdat) <- c("iter", "y", "omega")
	tmpdat$y	<- y.nodes[tmpdat$y]
	omega_deriv1<- spl2dfun(tmpdat, fml = omega ~ s(y) )
}else{
