...
for(i in 1:length(fmt_name)){
	...
	X_list[[i]]	<- cbind(tmp2, lag = tmp0, tmp1, tmp1*mydata$rec)
}
...
if(fixsigma){
	...
	theta_init	<- list(c(-.2, .03, -.2, -.06, .1, 	2, .2, .1, .2, 1.3, 		rep(-.1, length(selcol)), 1, -.2, 1, 1, -2,  	6, 34, 8, 8, 15, 60, -.45), 
						c(-.2, -.05,-.2,-.08,.1,	1.9, .3, .4, .5, 1, 		rep(-.1, length(selcol)),0, -.6, .8, .5, -1.5, 	6, 35, 8, 8, 18, 70, -.5), 
						c(-.3,-.12,-.38,-.15,.1,	1.8, .1, .4, .37, -.3, 	rep(-.1, length(selcol)),	-1, .7, -.5, 1.4, -1.2, 	5, 37, 8, 9, 20, 80, -.5 ) )
...
omega_parallel	<- function(eps_draw, lags, rec){
	...
	for(j in 1:length(lnInc)){
		...
		for(k in 1:R){
			...
			tmpX_list1[[k]]	<- rep(1, numnodes) %*% t(c(tmp, lags[k], tmpX_list[[k]], tmpX_list[[k]]*rec))
		}
...
tmp			<- foreach(i = 1:numsim, .packages= c("nloptr")) %dopar% {
	omega_parallel(eps_draw[i,], lags = lag_nodes[i,], rec = 0)
}
tmp1			<- foreach(i = 1:numsim, .packages= c("nloptr")) %dopar% {
	omega_parallel(eps_draw[i,], lags = lag_nodes[i,], rec = 1)
}
...
if(interp.method == "spline"){
	...
	names(tmpdat) <- c("iter", "lnInc", "y", "omega")
	tmpdat$Xlag	<- lag_nodes[tmpdat$iter,-beta0_base]						# Note that lags are perfectly correlated 
	omega_deriv1<- spl2dfun(tmpdat, fml = omega ~ te(lnInc, y) + Xlag)
}else{
