...
for(i in 1:R){
	...
	X_list07[[i]]	<- cbind(tmp1, lag = shr.stat[,i], rep(1, nrow(sim.unq)) %*% matrix(X_ls0[[i]], nrow = 1), matrix(0, nrow(sim.unq), length(selcol)))
	X_list08[[i]]	<- cbind(tmp2, lag = shr.stat[,i], rep(1, nrow(sim.unq)) %*% matrix(X_ls0[[i]], nrow = 1), rep(1, nrow(sim.unq)) %*% matrix(X_ls0[[i]], nrow = 1))
	colnames(X_list07[[i]])	<- colnames(X_list08[[i]])	<- c(fmt_name[-beta0_base], "lag",selcol, paste("rec*", selcol, sep=""))
}
...
pct				<- proc.time()
sim.base07		<- SimWrapper_fn(omega_deriv, ln_inc = sim.unq$Inc07, rec = rep(0, nrow(sim.unq)), 
...
pct				<- proc.time()
sim.base08		<- SimWrapper_fn(omega_deriv1, ln_inc = sim.unq$Inc08, rec = rep(1, nrow(sim.unq)), 
					lambda = lambda, param_est = shr.par, base = beta0_base, 
...
for(j in 1:length(sim.scn)){
	...
	for(k in 1:R){
		...
		X.new07[[k]]	<- cbind(tmp1, lag = shr.stat[,k], rep(1, nrow(sim.unq)) %*% matrix(X_ls0[[k]], nrow = 1), matrix(0, nrow(sim.unq), length(selcol)) )
		X.new08[[k]]	<- cbind(tmp2, lag = shr.stat[,k], rep(1, nrow(sim.unq)) %*% matrix(X_ls0[[k]], nrow = 1), 
															rep(1, nrow(sim.unq)) %*% matrix(X_ls0[[k]], nrow = 1) )
	}
	tmp2	<- tmp1	<- matrix(0, nrow(sim.unq), R)
	if(sel!=beta0_base){
		tmp1[,R]	<- 0
	...
	X.new07[[(R+1)]]	<- cbind(tmp1, lag = 0, rep(1, nrow(sim.unq)) %*% matrix(tmp, nrow = 1), matrix(0, nrow(sim.unq), length(selcol)))
	...
	newf.sim[[sidx]]	<- SimWrapper_fn(omega_deriv = omega_deriv1, ln_inc = sim.unq[,"Inc08"], 
				lambda = lambda, param_est = shr.par.new, 
