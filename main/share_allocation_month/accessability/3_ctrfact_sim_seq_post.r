...
for(ii in 1:nseg){
	...
	sim08			<- do.call(rbind, lapply(sim.prc.ls08, function(x) cbind(x$Average, obs = 1:nrow(x$Average))))
...
tmpbase	<- cbind(tmp07, income.level = 2007)
...
tmp		<- subset(tmpsim1, income.level == 2007)
tmp		<- tmp[order(tmp$retailer, tmp$variable, tmp$IncGrp, tmp$income2007),]
for(i in 1:R){
	tmp1<- compare_fn(tmp07, tmp[tmp$retailer == fmt_name[i],],
