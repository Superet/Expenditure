...
(fname	<- paste("estrun_",run_id,"/est_", seldpt, "_seg", seg_id, "_", Sys.Date(),".rdata",sep=""))
...
mydata		<- mydata[sel,]
tmp			<- data.table(hh_dpt[hh_dpt$department==dpt_name[seldpt],])
tmp			<- tmp[,list(nchannel = length(channel_type)), by = list(household_code, year)]
sel			<- tmp$nchannel == R
sel			<- paste(mydata$household_code, mydata$year, sep="*") %in% paste(tmp[sel,household_code], tmp[sel,year], sep="*")	
mydata		<- mydata[sel,]

# Filter out the households who face outlier retail attributes
tmp			<- data.table(subset(hh_dpt, department == dpt_name[seldpt]))
summary(tmp$unitprice_paid)
(tmp1		<- quantile(tmp$unitprice_paid, c(.001, .999)))
tmp			<- tmp[,list(keep = all(unitprice_paid>tmp1[1] & unitprice_paid<tmp1[2])), by = list(household_code)]
sum(tmp$keep)
tmp			<- subset(tmp, keep == TRUE)
length(unique(mydata$household_code))
dim(mydata)
mydata		<- subset(mydata, household_code %in% tmp$household_code)
length(unique(mydata$household_code))
dim(mydata)
...
tmp		<- dcast(hh_dpt[sel,c("household_code", "year", "channel_type","unitprice_paid")], 
...
hh_dpt$ln_num_module	<- log(hh_dpt$num_module)
hh_dpt$ln_upc_per_mod 	<- log(hh_dpt$num_upc_per_mod)
...
rm(list= intersect(ls(), c("hh_month", "Allocation_constr_fn","Allocation_fn","Allocation_nlop_fn","incl_value_fn","i","MDCEV_ll_fnC",
		  ...
		  "lastFuncGrad", "lastFuncParam", "args", "plot.wd", "tmpv1", "tmpv2", "tmpsol")))
