...
for(ii in 1:nseg){
	fname	<- paste("estrun_",run_id,"/fixb_est_", seldpt, "_seg", ii, "_", ver.date,".rdata",sep="")
	cat(fname, "\n")
	load(fname)
...
stargazer(tmp, type = "html", out = paste("estrun_", run_id,"/fixbest_",seldpt,"_", ver.date,".html", sep=""))
...
# Save the estimation 
save(exp.est.ls, shr.est.ls, omega.ls, fmt_name, hh_dist, mydata.full, nseg, pan, R, run_id, selcol, dpt_name, fmt_dpt, seldpt,
	file = paste("estrun_",run_id,"/fixb_est_",seldpt,"_", ver.date,".rdata",sep=""))
