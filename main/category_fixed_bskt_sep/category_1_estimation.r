...
mydata		<- mydata[sel,]
tmp 		<- data.table(fmt_dpt[fmt_dpt$department==dpt_name[seldpt],])
tmp 		<- tmp[,list(nchannel = length(channel_type)), by = list(scantrack_market_descr, year)]			
...
# Match price
sel		<- fmt_dpt$department == dpt_name[seldpt]
...
sel		<- fmt_dpt$department == dpt_name[seldpt]
tmp		<- merge(fmt_dpt[sel,], hh_dist[,c("household_code", "scantrack_market_descr", "year", "channel_type", "panelist_zip_code","distance_haver")], 
