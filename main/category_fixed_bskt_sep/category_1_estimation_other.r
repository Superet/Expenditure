...
dpt_name	<- setNames(c("DRY GROCERY","GENERAL MERCHANDISE", "NON-FOOD GROCERY", "PRODUCE OTHER", "REFRIGERATED FROZEN", "HEALTH BEAUTY CARE"), 
...
# NOTE: the measurement is NOT great for product & others, not much variation. 
ggtmp <- subset(fmt_dpt, department == "PRODUCE OTHER")
ggtmp1 <- melt(ggtmp[,c("scantrack_market_descr", "year", "channel_type", "unitprice_paid", "num_module", "num_upc_per_mod", "size_index","prvt_overall")], 
				id.vars = c("scantrack_market_descr", "year", "channel_type")) 
# ggplot(ggtmp1, aes(channel_type, value)) + geom_boxplot() + facet_wrap(~variable, scales = "free")

# ------------#
# Subset data #
# Subset the segment 
selcol		<- c("household_code", "year", "month", "rec", paste(seldpt, ".", fmt_name, sep=""))
...
sum(sel)/length(sel)
mydata		<- mydata[sel,]
mydata$dol	<- rowSums(as.matrix(mydata[,grep(seldpt, names(mydata))]))								
sum(mydata$dol < .1)
mydata.ful	<- mydata
...
shr		<- as.matrix(mydata[,paste(seldpt,".", fmt_name,sep="")])/y
ln_inc	<- log(mydata$income_mid)
tmp 	<- 1 *(shr> 0); colnames(tmp)	<- NULL
s1_index<- c(apply(tmp, 1, which.max))
price	<- matrix(1, nrow(mydata), R)

# Match retailers' attributes
selcol	<- c("distance_haver")
...
tmp		<- data.table(hh_dist[,c("household_code", "scantrack_market_descr", "year", "channel_type", "panelist_zip_code","distance_haver")])
...
for(i in 1:length(fmt_name)){
	...
	tmp	<- as.matrix(attr_mat[sel,selcol]); colnames(tmp)	<- selcol
...
nx 		<- ncol(X1[[1]])
cat("nx =", nx,"\n")
rm(list = intersect(ls(), c("tmp1", "tmp2", "tmp3", "tmp", "tmp0")))
...
myfix 	<- length(theta.init[[1]])

max.method	<- "BFGS"	#"NM"
tmpsol	<- vector("list", length = length(theta.init))
for(i in 1:length(theta.init)){
	pct <- proc.time()
	tmpsol[[i]]		<- maxLik(MDCEV_wrapper, start=theta.init[[i]], method=max.method, fixed = myfix)
	use.time <- proc.time() - pct
...
tmp		<- quantile(y, c(.05, .95))
newx	<- data.frame(y = seq(tmp[1], tmp[2], length = 100))
...
rm(list = intersect( ls(), c("sht.X1", "sht.X2", "sht.price", "sht.y", "tmpdat", "newx", "tmpm")))
...
rm(list= intersect(ls(), c("hh_month", "Allocation_constr_fn","Allocation_fn","Allocation_nlop_fn","incl_value_fn","i","MDCEV_ll_fnC",
		  ...
		  "lastFuncGrad", "lastFuncParam", "args", "plot.wd", "tmpv1", "tmpv2", "tmpsol", "model_name", "omega.lm", "u", "spl2dfun")))
