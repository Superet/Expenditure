...
dpt		<- c("DG", "GM","NFG", "OTHER","RF", "HBC")
dpt_id	<- as.numeric(Sys.getenv("PBS_ARRAY_INDEX"))
seldpt	<- dpt[dpt_id]
cat("seldpt =", seldpt, ".\n")
# 
...
ver.date	<- "2016-09-26"
loadf	<- paste("estrun_",run_id,"/fixb_est_", seldpt,"_", ver.date,".rdata",sep="")
loadf
load(loadf)
...
draw.par		<- FALSE
sim.omega		<- FALSE
fname			<- paste(seldpt,"_marginal_effect_sep",sep="")
...
rm(list = intersect(ls(), c("fmt_dpt", "hh_month", "X1", "X2", "attr_mat", "pan")))
