...
SimWrapper_fn	<- function(omega_fn_ls, ln_inc, lambda, param_est, base, X_list, price, eps_draw, 
							sim.y = NULL, method = "FOC", use.bound = FALSE, ret.sim = FALSE){
...
#=========================================================================================================#
	...
	for(i in 1:length(lvinc)){
		...
		if(sum(sel) > 0){
			...
			for(j in 1:numsim){
				tmpsol 		<- try(incl_value_fn(param_est, base, X_list=tmpX_list1, y=y[sel], Q=Inf, price=rep(1, sum(sel)) %*% t(price), 
	...
	out	<- cbind(y, colMeans(omega, na.rm = T), colMeans(omega1, na.rm = T), apply(e, c(2,3), mean, na.rm= T))	
	colnames(out)	<- c("Exp", "omega", "spl.omega", fmt_name)
	# if(!is.null(base.ls)){
	# 	d	<- lapply(base.ls, function(x){apply(e-x, c(2, 3), sd, na.rm=T)})
	# 	d	<- do.call(cbind, d)
	# 	colnames(d)	<- paste("dif.se.", rep(names(base.ls), each=R), ".",rep(fmt_name, length(base.ls)), sep = "")
	# 	out	<- cbind(out, d)
	# }
	if(ret.sim){
...
SimOmega_fn	<- function(ln_inc, lambda, param_est, base, X_list, price, lnInc_lv, y.nodes, eps_draw, 
						method, interp.method, ret.sim, alpha = 0.05){
...
#===================================================================================================#
	...
	if(interp.method == "spline"){
		...
		if(!is.list(tmp1)){
			...
			omega.ls.new	<- vector("list", length(lnInc_lv))
			for(i in 1:length(lnInc_lv)){
				...
				omega.ls.new[[i]]	<- mysplfun(x = tmpx, y = unlist(tmp1))
			}
		}			
	}else{
	...
	out	<- SimWrapper_fn(omega_fn_ls = omega.ls.new, ln_inc = ln_inc, lambda = lambda, param_est = param_est, 
						base = base, X_list = X_list, price = price, eps_draw = eps_draw, method = method, ret.sim = ret.sim)
	return(out)
}
