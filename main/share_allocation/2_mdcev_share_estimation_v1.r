...
theta_init	<- list(c(-1, -.1, 1, -2, .1, .1, -.1, .1, -1, -1, -1, -.5, -2, 3, 15, 4, 3, 6, 33, -.3), 
					c(-.6, -.2, 1.1, -.8, .1, .1, -.1, -.2, -1.2, -1, -1, -.5, -.5, 6, 25, 5, 6, 10, 60, -.4), 
					c(-1, -.3, 1.2, -6, .1, .1, -.1, .6, -1.5, -1, -1.5, -.7, -.3, 5, 30, 6, 6, 15, 80, -.4 ),
					c(-1, -.2, 1.1, -4, .1, 0, -.1, .2, -1.5, -1, -1.5, -.7, -.3, 6, 25, 5, 6, 10, 80, -.4  ) )
...
tmp		<- apply(omega_draw, c(2, 3), mean, na.rm =T, trim = trim.alpha)
if(interp.method == "spline"){
	...
	omega_deriv	<- lapply(1:length(lnInc), function(i) 
