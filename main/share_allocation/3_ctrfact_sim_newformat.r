...
X_list07	<- lapply(X_ls0, function(x) 
...
for(i in 1:length(ret.a)){
	...
	shr.par.new	<- c(shr.par[1:nx], shr.par[grep("beta0", names(shr.par))], beta0_7, 
	...
	for(j in 1:length(inherent.price)){
		# Set new attributes
		X.new	<-	X_list07
		X.new$new	<- X_list07[["Convenience Store"]]
		X.new$new[,"overall_prvt"]	<- X_list07[[ret.a[i]]][,"overall_prvt"]
		# if(inherent.prvt[j]){
		# 	X.new$new[,"overall_prvt"]	<- X_list07[[ret.a[i]]][,"overall_prvt"]
		# }
		price.new		<- price.07
		...
		newf.sim07[[sidx]]	<- SimWrapper_fn(omega_deriv = omega_deriv, ln_inc = sim.unq[,"Inc07"], 
					...
					base = beta0_base, X_list = X.new, price = price.new, sim.y = sim.base07$y, 
		...
		newf.sim[[sidx]]	<- SimWrapper_fn(omega_deriv = omega_deriv, ln_inc = sim.unq[,"Inc08"], 
					lambda = lambda, param_est = shr.par.new, 
					base = beta0_base, X_list = X.new, price = price.new, sim.y = sim.base08$y, 
					eps_draw = eps_draw_new, method = exp.method, ret.sim = TRUE, par.draw = par_draw)
	}
	use.time	<- proc.time() - pct
