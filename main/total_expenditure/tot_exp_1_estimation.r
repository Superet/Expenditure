...
# Add lagged expenditure
mydata	<- data.table(mydata)
setkeyv(mydata, c("household_code", "year", "month"))
mydata	<- mydata[, lag_dol:= c(NA, dol[-length(dol)]), by = list(household_code)]
mydata	<- data.frame(subset(mydata, !is.na(lag_dol)))
...
mydata	<- merge(mydata, pan[,c("household_code", "panel_year", "income_mid", "household_size")], 
...
theta.init	<- list(c(beta.init, gamma.init/100, -.5), 
					c(-.1, -.08, -.1, -.05, -.1, 	.03, .005,.04, .02, .04,		-.04, 0, .05, .01, .2,		.001, -.0015, .01, -.02, -.06, 
					  .2, .3, .25, .25, .3, .4, 	-.02, -.05, -.05, -.05, -.05, -.05, 	.01, -.003, .01, -.007, -.25, 	-.002, .0002, .007, .002, .12, -1))
...
if(interp.method == "spline"){
	omega_deriv	<- spl2dfun(tmpdat, fml = omega ~ s(y) + price + X)
}else if(interp.method == "lm"){
	omega_deriv	<- omega.lm(tmpdat, fml = omega ~ I(y^2) + y + price + price*y + X)
}
...
# Moment function: lambda * omega'(y,Inc) - 1 = 0
	m1	<- (lambda[1] + lambda[2] * rec + lambda[3]*lag_dol)* o.deriv - 1
	m	<- cbind(m1, m1*ln_inc, m1*mydata$household_size)
	mbar<- colMeans(m)
...
lsol	<- lm(1/o.deriv ~ factor(rec) + lag_dol)
pct		<- proc.time()
init	<- matrix(c(coef(lsol), 
					-10, -2, .1, 
					-5, -1, .1), 
					3, 3, byrow = T)
colnames(init)	<- c("lambda1", "lambda2", "lambda3")
dT		<- NULL
