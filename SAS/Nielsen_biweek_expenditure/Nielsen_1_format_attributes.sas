...
%macro read_groc_fun;
		...
		proc sql noprint; 
			...
			* Subset purchases data within restrictions;
			...
			select A.*, cats(upc, "-", upc_ver_uc) as upcv, (total_price_paid-coupon_value) as dol_paid, total_price_paid/quantity as price,
					B.scantrack_market_descr, B.channel_type, B.retailer_code, B.panel_year, 
			...
			* Aggretate individual transaction to biweekly data at upc level;
			create table biweek_purchase&i as
			select scantrack_market_descr, channel_type, retailer_code, upcv, year, biweek,
					mean(upc) as upc, mean(upc_ver_uc) as upc_ver_uc, 
					sum(total_price_paid) as dol, sum(quantity) as quantity,
					sum(dol_paid) as dol_paid, sum(total_price_paid)/sum(quantity) as price_tag, sum(dol_paid)/sum(quantity) as price_paid
			from tmp3
			group by scantrack_market_descr, channel_type, retailer_code, upcv, year, biweek;
		quit;
	...
	* Combine biweekly transaction;
	data biweek_transaction;
		set biweek_purchase2004 - biweek_purchase2012;
	run;
	
	proc datasets noprint; 
		delete panelists2004 - panelists2012 annual_purchase2004 - annual_purchase2012 trips2004 - trips2012 biweek_purchase2004 - biweek_purchase2012; 
	run;
%mend read_groc_fun;
...
************************************************;
* ------------------------------------------------------------------------------------------ *;
* Change product modules in MAGNET DATE to other DEPARMENTS;
proc sql noprint;
	create table tmp1 as 
	select *
	from orig_products
...
proc sql noprint;
	...
	select *, B.module_new, B.group_new, B.department_new
...
proc sql noprint;
	...
	* Sum up dollars spent on each module in every market;
	create table tmp1 as 
	select scantrack_market_descr, product_module_descr, sum(dol) as dol, sum(dol_paid) as dol_paid
	from tmp
	group by scantrack_market_descr, product_module_descr;
	
	* Compute wallet share of each module in every market;
	create table module_wallet as
	select scantrack_market_descr, product_module_descr, dol/sum(dol) as share, dol_paid/sum(dol_paid) as share_paid
	from tmp1 
	group by scantrack_market_descr
	order by scantrack_market_descr, product_module_descr;
quit;
...
proc sql noprint;
	* Use average transaction volume to compute weights associated with each upc;
	create table tmp as
	select  scantrack_market_descr, year, channel_type, retailer_code, upcv, mean(upc) as upc, 
	...
	group by scantrack_market_descr, year, channel_type, retailer_code, upcv;
...
/*	* Aggregate to retailer level weighted by product module scan units;
	create table retailer_size as
	select scantrack_market_descr, channel_type, retailer_code,
	...
	group by scantrack_market_descr, channel_type, retailer_code;*/
quit;
...
*************************************************************************;
proc sql noprint;
	* Average annual quantity of UPC in a channel type in a scantrack_market_descr;
	create table tmp as 
	select scantrack_market_descr, channel_type, upcv, mean(upc) as upc, mean(upc_ver_uc) as upc_ver_uc,
			mean(quantity) as quantity, mean(dol) as revenue
	from annual_purchase
	group by scantrack_market_descr, channel_type, upcv;
	...
	* Compute channel-module index weighted by module units;
	...
	select scantrack_market_descr, channel_type, department_descr, product_group_descr, product_module_descr, 
	...
	from tmp1 
	group by scantrack_market_descr, channel_type, department_descr, product_group_descr, product_module_descr;
	
	* Aggregate channel-level index using wallet share as module weight;
	create table channel_size1 as 
	select scantrack_market_descr, channel_type, sum(size_index_volwt*share_paid)/sum(share_paid) as size_index,
		sum(num_brands) as num_brands, sum(num_brands*share_paid)/sum(share_paid) as avg_brands_unitwt,
		sum(num_upc) as num_upc, sum(num_upc*share_paid)/sum(share_paid) as avg_upc_unitwt,
		count(distinct product_module_descr) as num_module
	from (select A.*, B.share_paid from channel_mod1 as A left join module_wallet as B 
		  on A.scantrack_market_descr=B.scantrack_market_descr and A.product_module_descr = B.product_module_descr)
	group by scantrack_market_descr, channel_type
	order by scantrack_market_descr, channel_type;
quit; 
...
proc sql noprint;
	...
	select *
	from tmp1 
	where scantrack_market_descr=&sel_mkt and product_module_descr = &sel_mod
...
*********************************;
* Construct biweekly price index *;
*********************************;
* Compute the biweekly price index for each retailer at each scantrack_market_descr;
proc sql noprint;
	* Append the upc-related information from products data;
	create table retailer_upc as 
	select A.*, B.department_descr, B.product_group_descr, B.product_module_descr, B.brand_descr, B.size_index,
	...
	from biweek_transaction as A inner join products as B
	on A.upcv = B.upcv;
	
	* Aggregate to retailer-module level weighted by UPC volumn at each retailer;
	create table retailer_module as 
	select scantrack_market_descr, year, biweek, channel_type, retailer_code, department_descr, product_group_descr, product_module_descr,
	...
	from retailer_upc
	group by scantrack_market_descr, year, biweek, channel_type, retailer_code, department_descr, product_group_descr, product_module_descr;
	
	* Aggregate retailer level index using wallet share as module weight;
	* NOTE: WE USE ANNUAL WALLET SHARE TO WEIGHT biweekLY PRICES ACROSS CATEGORIES; 
	create table retailer_index as 
	select scantrack_market_descr, year, biweek, channel_type, retailer_code,
		...
		sum(price_tag_volnorm)/sum(share_paid) as bsk_price_tag, 
		sum(price_paid_volnorm)/sum(share_paid) as bsk_price_paid
	from (select A.*, B.share_paid from retailer_module as A left join module_wallet as B 
		  on A.scantrack_market_descr=B.scantrack_market_descr and
		  A.product_module_descr = B.product_module_descr)
	group by scantrack_market_descr, year, biweek, channel_type, retailer_code
...
proc sql noprint;	
	* Annual number of trips and revenue for each retailer;
	...
	select scantrack_market_descr, year, biweek, channel_type, retailer_code, 
			count(trip_code_uc) as num_trips, sum(net_spent) as revenue
	from (select *, ceil((purchase_date - MDY(12,31,2003))/14) as biweek from trips )
	group by scantrack_market_descr, year, biweek, channel_type, retailer_code;
	...
	* Append retailer transaction information to the price index at retailer level;
	...
	on A.scantrack_market_descr=B.scantrack_market_descr and A.year=B.year 
		and A.biweek=B.biweek and A.retailer_code = B.retailer_code;
	...
	* Channel aggregation: Aggregate to market-channel level from retailer data weighted by store revenue;
	...
	select scantrack_market_descr, year, biweek, channel_type, 
	...
	from tmp
	group by scantrack_market_descr, year, biweek, channel_type
