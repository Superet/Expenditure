		...
		* Import purchase data;
		...
		proc sql noprint; 
			...
			* Aggretate individual transaction to monthly data at upc level;
			...
			select scantrack_market_descr, channel_type, retailer_code, upcv, year, month,
					mean(upc) as upc, mean(upc_ver_uc) as upc_ver_uc, 
					sum(total_price_paid) as dol, sum(quantity) as quantity,
					sum(dol_paid) as dol_paid, sum(total_price_paid)/sum(quantity) as price_tag, sum(dol_paid)/sum(quantity) as price_paid
			from tmp3
			group by scantrack_market_descr, channel_type, retailer_code, upcv, year, month;
		quit;
...
*********************************;
* Compute the monthly price index for each retailer at each scantrack_market_descr;
proc sql noprint;
	* Append the upc-related information from products data;
	create table retailer_upc as 
	select A.*, B.department_descr, B.product_group_descr, B.product_module_descr, B.brand_descr, B.size_index,
	...
	* Aggregate to retailer-module level weighted by UPC volumn at each retailer;
	create table retailer_module as 
	select scantrack_market_descr, year, month, channel_type, retailer_code, department_descr, product_group_descr, product_module_descr,
	...
	from retailer_upc
	group by scantrack_market_descr, year, month, channel_type, retailer_code, department_descr, product_group_descr, product_module_descr;
	
	* Aggregate retailer level index using wallet share as module weight;
	* NOTE: WE USE ANNUAL WALLET SHARE TO WEIGHT MONTHLY PRICES ACROSS CATEGORIES; 
	create table retailer_index as 
	select scantrack_market_descr, year, month, channel_type, retailer_code,
	...
	group by scantrack_market_descr, year, month, channel_type, retailer_code
	order by scantrack_market_descr, year, month, channel_type, retailer_code;
quit;
...
proc sql noprint;	
	...
	* Channel aggregation: Aggregate to market-channel level from retailer data weighted by store revenue;
	...
	select scantrack_market_descr, year, month, channel_type, 
	...
	order by scantrack_market_descr, year, month, channel_type;
quit;
